 window.addEventListener('load' , function(){
    ...
    video.addEventListener('canplay' , function(){
    	...
    	updatePlayer();  // see actual video duration on load
 ...
 function skip(ev){
 	...
 	//offsetLeft property returns the left position (in pixels) 
 	//relative to the left side the offsetParent element

 	//convert progress bar in pixel
 ...
 function getFormattedTime(){
 	var seconds = Math.round(video.currentTime); // time in seconds
 	var minutes = Math.floor(seconds/60); //time in minute
 	if (minutes > 0) seconds = seconds-(minutes*60);
 	if (seconds.toString().length === 1) seconds = '0' + seconds;

 	var totalSeconds = Math.round(video.duration);
 	var totalMinutes = Math.floor(totalSeconds/60);
 	if (totalMinutes > 0) totalSeconds = totalSeconds-(totalMinutes*60);
 ...
 function muteOrUnmute(){
 	if (!video.muted) {
 		...
 		sbar.style.display = 'none'; // disappear sbar-progress when muted video
 	} 
